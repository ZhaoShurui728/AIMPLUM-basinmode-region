...
function ScnMergeRun() {
  ...
  read -p "push any key";
  if [ ${IAV} = "NoCC" ]; then gams ../$1/prog/protect_area_agg.gms --prog_loc=$1 --SCE=${SCE} --CLP=${CLP} --IAV=${IAV} --ModelInt2=${ModelInt2} MaxProcDir=100 o=../output/lst/protect_area_agg_$2.lst  lo=4
  read -p "push any key";
