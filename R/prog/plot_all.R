...
regionin <-  c("WLD")
regionin <-  c("USA")
yearin <-  c("2005","2010","2030","2050","2080","2100")
yearin <-  c("2050","2070")
yearin <-  c("2050")
scein <- c("SSP1","SSP2","SSP3","SSP4","SSP5")
scein <- c("SSP3")
clpin <- c("BaU")
iavin <- c("NoCC")
...
for(m in (1:length(scein))){
  ...
  if (scename=="SSP1"){
    clpin <- c("BaU","20W","26W","37W","45W")
  } else if (scename=="SSP2"){
    clpin <- c("BaU","20W","26W","37W","45W","60W")
  } else if (scename=="SSP3"){
    clpin <- c("BaU","37W","45W","60W")
  } else if (scename=="SSP4"){
    clpin <- c("BaU","26W","37W","45W")
  } else if (scename=="SSP5"){
    clpin <- c("BaU","26W","37W","45W","60W")
  } else {
  }
...
load_filename <- paste('../../../output/txt/',scein[m],'_',clpin[n],'_',iavin[l],'/',regionin[r],'/',yearin[t],'.txt', sep = '')
	...
	#porg <- pdata0[pdata0$Variable==c("PDRIR")|pdata0$Variable==c("PDRRF")|
	#                 pdata0$Variable==c("WHTIR")|pdata0$Variable==c("WHTRF")|
	#                 pdata0$Variable==c("GROIR")|pdata0$Variable==c("GRORF")|
	#                 pdata0$Variable==c("C_BIR")|pdata0$Variable==c("C_BRF")|
	#                 pdata0$Variable==c("OSDIR")|pdata0$Variable==c("OSDRF")|
	#                 pdata0$Variable==c("OTH_AIR")|pdata0$Variable==c("OTH_ARF")|
	#                 pdata0$Variable==c("BIO")|pdata0$Variable==c("CROP_FLW"), c(1,2,3,4)]
...
#		out_filename <- paste('../../../output/png/',scein[m],'_',clpin[n],'_',iavin[l],'/',regionin[r],'/',scein[m],'_',clpin[n],'_',iavin[l],'_',yearin[t],'.png', sep = '')
		out_filename <- paste('../../../output/png/',scein[m],'_',clpin[n],'_',iavin[l],'_',regionin[r],'_',yearin[t],'.png', sep = '')
#		out_filename <- paste('../../../output/png/',scein[m],'_',clpin[n],'_',iavin[l],'_',yearin[t],'_',regionin[r],'byCrops2.png', sep = '')
...
#		facet_wrap(~Variable,ncol = 2) + 
 ...
 ggsave(file = out_filename, plot = plot3, dpi = 300, width = 10, height = 10)
