...
A2Fun <- function(BASE_AOO_interp, genlentable){

	A2_AOO_interp_rl <- BASE_AOO_interp
	for(A2.rownum in seq(1,nrow(A2_AOO_interp_rl))) {
	  A2.taxid <- as.integer(A2_AOO_interp_rl[A2.rownum,1])
	    ...
	    for(A2.colnum in seq(2,ncol(A2_AOO_interp_rl))) {
	        ...
	        if(BASE_AOO_interp[A2.rownum,A2.colnum] == 0) {A2_AOO_interp_rl[A2.rownum,A2.colnum:ncol(A2_AOO_interp_rl)] <- 5; break} else{
