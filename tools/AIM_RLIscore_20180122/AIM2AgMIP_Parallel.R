# create a cluster for multicore processing
CL1 <- makeCluster(p_run)
...
convertFun <- function(version){
  ...
  # send objects to clusters created, use the flowing to send object from local environment
  clusterExport(CL1, c('gridsize0.5', 'gridsize2', 'countrygridAgMIP', 'basegrid','AIMLUT'))
  clusterExport(CL1, c('inputdir', 'outputdir', 'pdfdir','gdxfiles'), envir = env)
  
  # use it to load libraries which are required in the cluster
  clusterEvalQ(CL1, 
               {
                library(dplyr)
                library(raster)
  ...
  # cluster computation
  clusterApply(CL1, gdxfiles, fun = convertSingle)
  print(Sys.time())
