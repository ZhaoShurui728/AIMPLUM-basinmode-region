...
# Switch & Settings ---------------------------------
  ...
  weightBy <- c("prod") # write area or prod 
  print(paste("weighting by", weightBy))
  ...
  inputFile <- paste("AIM-LUmap_",SceName,".nc",sep="")
  cat("Input file name is", inputFile, "\n")
  landuseFile <- grep(inputFile, allncFiles, value = TRUE, ignore.case = TRUE)
  # display your input file
  cat("Input file is", landuseFile, "\n")

  # Analysis region (IPBESsubregion or AIM17regions)
  regionUnit <- 'AIM17regions'
  cat("Aggregate region is", regionUnit, "\n")
  ...
  BIIcoefsFile <- read.csv(paste0(BIICoefsDir,"BII_coefs.csv"))

# LOAD SIDE DATA ----------------------------------------------------------

  # -- Potential NPP layer (derived from Del Grosso et al. 2008, Ecology)
   potNPP_ncfile <- nc_open(paste0(supplementaryDir,'potNPP_corrected_updated.nc'))
  ...
  # -- link from pixels to regions
   nc_file <- nc_open(paste0(supplementaryDir,'Link_halfdegreeGrid_to_',regionUnit,'_R2.nc'))
   mylon <- ncvar_get(nc_file, "lon")
   mylat <- ncvar_get(nc_file, "lat")

  if(regionUnit == 'AIM17regions'){
       myreg <- ncvar_get(nc_file, "aim_17region")
  } else if (regionUnit == 'IPBESsubregion') {
...
# PREPARE BII COEFFICIENTS ------------------------------------------------
  ...
  # -- BII coefficients per LU class
    ...
    in_PREDICTS_coefs <- BIIcoefsFile
...
# PROCESS -----------------------------------------------------------------
    ...
    # -- Landuse data process spliting each grid to potentially-forest & potentially-nonforest

    if(exists('collector_agg_values')){rm('collector_agg_values')};for(cur_file in landuseFile) {
    ...
    # - reformat and store
      ...
      temp_rf <- pivot_longer(output_template, cols = c(value_area, value_prod), names_to = 'variable', values_to = 'value')
      ...
      output_template_rf <- temp_rf %>% 
        mutate(lc_class = paste0(lc_class,'_',fnf)) %>% 
        dplyr::select(-c(fnf)) %>% 
        pivot_wider(names_from = lc_class, values_from = value)
...
#  # -- ALL AT ONCE
    write.table(collector_agg_values_bis, paste0(landuseDir,"aggregated_regions_forPREDICTS_R2_final.csv",sep=""),col.names = T, row.names = F, quote = F,sep=',')
...
# PROCESS2 -----------------------------------------------------------------
  ...
  # -- merge 'area or prod' subset with BII value, multiply & aggregate
    ...
    to_join <- agg2 %>% 
      filter(variable == weightBy) %>% 
      dplyr::select(-c(variable)) %>% 
      pivot_longer(cols = -c('region','year','scenario','sum'), names_to = 'variable', values_to = 'value')
...
# EXPORT ------------------------------------------------------------------
  write.table(output,paste0(BIIOutputDir,"csv/BII_regionagg_",SceName,"_",PRJ,".csv",sep = ""),col.names = T, row.names = F, quote = F, sep=',')
  ...
  wgdx.lst(paste0(BIIOutputDir,"gdx/BII_regionagg_",SceName,".gdx"), output_gdx)
