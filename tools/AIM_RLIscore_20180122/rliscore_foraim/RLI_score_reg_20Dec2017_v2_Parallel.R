...
clusterExport(CL2, c('inputdir','outputfile','specieslanduse', 'specieslanduse', 'file_df', 
					'test', 'years', 'regions','link_AgMIP_Reg_ID_LUID', 'vars_output',
					'common_set', 'common_set_save', "genlentable",'files_by_scenario','outputs',           
					"nsites", "nspecies", "speciesbysite", "specieslanduse", 'interpolate', 'gm_mean'), envir = env)

RLIfun <- function(curfile){

  reg_template_output <- cbind.data.frame(id_file = curfile, expand.grid('reg'=regions,'variable'=vars_output,'year'=years), 'val'=NA, file = files_by_scenario[curfile])  
  ...
  return(reg_template_output)
}

CL2_result <- clusterApply(CL2, file_df$id_file, RLIfun)

output <- NULL
for(i in 1:length(file_df$id_file)){
	output <- rbind(output, CL2_result[[i]])
}
