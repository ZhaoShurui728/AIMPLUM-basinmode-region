...
packagesList <- c('raster', 'dplyr', 'data.table', 'reshape2', 'ggplot2', 
                  'stacomirtools', 'qdap', 'lazyeval', 'parallel')
...
# detect the number of thread in your CPU
(p <- detectCores())
# set the number of thread you want to use, 
# if it makes your computer too slow, please reduce the number of thread
p_run <- p - 0
...
source("AIM2AgMIP_Parallel.R") 
convertFun(version)

# 2. Calculate indices
# the output index file is 
# \RLIscore_forAIM\output\BiodivIndicator_trends_AIM_version.csv
source('RLIscore_forAIM\\RLI_score_reg_20Dec2017_v2_Parallel.R')
indexFun(version)
