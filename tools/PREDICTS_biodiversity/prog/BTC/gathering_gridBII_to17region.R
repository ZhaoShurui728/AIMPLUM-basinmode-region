...
# -- settings of year and variables# -- Arguments from shell script  
...
BIIresultFileName <- paste0("BIImap_",SceName,"_",PRJ,".nc")
...
# load data 
 ...
 # -- Get pixel area from AIM/PLUM output
   nc_file <- nc_open(paste0(landuseFileName))
 ...
 # -- files to be looped upon
   ...
   # region by region, compute area and prod
     for(cur_reg in seq(1,length(myreg))) {
       ...
       print(paste("Now calculate region number", cur_reg ,'/17'))
