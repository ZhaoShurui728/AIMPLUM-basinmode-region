...
if (Climate_sce != 'none'){
  calYear <- c(2005,2010,2030,2050,2090)
} else {
...
ab_m_selected <- lmer(sqrt(RescaledAbundance) ~ LandUse + (1|SS) + (1|SSB), data = TotalAbundance)
...
cs_m_selected <- lmer(logitCS ~ lu_contrast + log10geo + cubrtEnvDist + lu_contrast:cubrtEnvDist + lu_contrast:log10geo + (1|SS) + (1|s2), data = CompositionalSimilarity) 
...
Cs.coef <- data.frame(lu_contrast = levels(CompositionalSimilarity$lu_contrast), log10geo = 0, cubrtEnvDist = 0) %>% 
