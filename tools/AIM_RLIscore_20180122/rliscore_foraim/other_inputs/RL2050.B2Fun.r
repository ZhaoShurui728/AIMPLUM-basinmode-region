...
B2Fun <- function(BASE_AOO_interp){
	...
	for(B2.rownum in seq(1,nrow(B2_AOO_interp_rl)))
	{
	  for(B2.colnum in seq(2,ncol(B2_AOO_interp_rl)))
	  {
	    ...
	    if(    BASE_AOO_interp[B2.rownum,B2.colnum-B2.BoolWindow] == 0) {B2.BoolDecline <- FALSE} else {
	           if(BASE_AOO_interp[B2.rownum,B2.colnum]/BASE_AOO_interp[B2.rownum,B2.colnum-B2.BoolWindow] <= 0.99 ) {B2.BoolDecline <- TRUE} else {B2.BoolDecline <- FALSE}}
